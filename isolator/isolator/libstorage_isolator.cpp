...
const char LibstorageIsolator::prohibitedchars[NUM_PROHIBITED]  =
{
  '%', '/', ':', ';', '\0',
  '<', '>', '|', '`', '$', '\'',
  '?', '^', '&', ' ', '{', '\"',
...
string LibstorageIsolator::mountPbFilename;
string LibstorageIsolator::mesosWorkingDir;


LibstorageIsolator::LibstorageIsolator(
...
Try<Isolator*> LibstorageIsolator::create(
    const Parameters& parameters)
{
  // Check for root permission.
  if (geteuid() != 0) {
    return Error("The 'libstorage' isolator requires root permissions");
  }

  LOG(INFO) << "LibstorageIsolator::create() called";
  mesosWorkingDir = DEFAULT_WORKING_DIR;

  foreach (const Parameter& parameter, parameters.parameter()) {
    if (parameter.key() == LIBSTORAGE_WORKDIR_PARAM_NAME) {
      ...
      if (parameter.value().length() > 1 &&
        ...
        std::stringstream ss;
        ss << "LibstorageIsolator " << LIBSTORAGE_WORKDIR_PARAM_NAME
  ...
  mountPbFilename = path::join(getMetaRootDir(mesosWorkingDir),
                                 LIBSTORAGE_MOUNTLIST_FILENAME);
  LOG(INFO) << "using " << mountPbFilename;

  return new LibstorageIsolator(parameters);
}
...
Future<Nothing> LibstorageIsolator::recover(
    const list<ContainerState>& states,
    const hashset<ContainerID>& orphans)
{
  LOG(INFO) << "LibstorageIsolator recover() was called";
  ...
  LOG(INFO) << "libstoragecheckpoint::recover() called";
  Result<State> resultState =
    mesos::internal::slave::state::recover(mesosWorkingDir, true);

  State state = resultState.get();
  LOG(INFO) << "libstoragecheckpoint::recover() returned: " << state.errors;
  ...
  //checkpoint the libstorage mounts for persistence
...
bool LibstorageIsolator::unmount(
...
string LibstorageIsolator::mount(
    const ExternalMount& em,
    const string&   callerLabelForLogging) const
{
  LOG(INFO) << em.volumedriver() << "/" << em.volumename()
...
bool LibstorageIsolator::containsProhibitedChars(
    const string& s) const
{
  return (string::npos != s.find_first_of(prohibitedchars, 0, NUM_PROHIBITED));
}

bool LibstorageIsolator::parseEnvVar(
...
Failure LibstorageIsolator::revertMountlist(
  ...
  for (size_t i = 0; i < volumeNames.size(); i++) {
    ...
    if (!mountInUse) {
      ...
      if (!containerPaths[i].empty() &&
        ...
        if (mkdir.isError()) {
          return Failure(
            "LibstorageIsolator could not create container path dir: " +
            containerPaths[i]);
        }
      }
    }
...
Future<Nothing> LibstorageIsolator::update(
    const ContainerID& containerId,
...
Future<ResourceStatistics> LibstorageIsolator::usage(
    const ContainerID& containerId)
{
...
Future<Nothing> LibstorageIsolator::isolate(
    const ContainerID& containerId,
    pid_t pid)
{
...
Future<Nothing> LibstorageIsolator::cleanup(
    const ContainerID& containerId)
{
...
static Isolator* createLibstorageIsolator(const Parameters& parameters)
{
  LOG(INFO) << "Loading Libstorage Isolator module";

  Try<Isolator*> result = LibstorageIsolator::create(parameters);
...
// Declares the isolator named com_emccode_mesos_LibstorageIsolator.
mesos::modules::Module<Isolator> com_emccode_mesos_LibstorageIsolator(
    MESOS_MODULE_API_VERSION,
    MESOS_VERSION,
    "emc{code}",
    "emccode@emc.com",
    "Libstorage Isolator module.",
    NULL,
    createLibstorageIsolator);
